                ...
                <v-table>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="whitespace-nowrap" label="포트폴리오">{{ val.portfolioUrl }}</td>
                            <td class="whitespace-nowrap" label="대표작">{{ val.masterpiece }}</td>
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <div>
                            ...
                            <String label="포트폴리오" v-model="selectedRow.portfolioUrl" :editMode="true"/>
                            <String label="대표작" v-model="selectedRow.masterpiece" :editMode="true"/>
                            <String label="승인여부" v-model="selectedRow.isApproved" :editMode="true"/>
    ...
    methods:{
        ...
        async disapproveAuthor(params){
            try{
                ...
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','disapproveAuthor 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
